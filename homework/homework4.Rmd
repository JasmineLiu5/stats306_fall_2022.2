...
```{r}
cancer_deaths2<- select(cancer_deaths,-`All other and unspecified malignant neoplasms (C17,C23-C24,C26-C31,C37-C41,C44-C49,C51-C52,C57-C60,C62-C63,C66,C68-C69,C73-C80,C97)`)
cancer_deaths2
#There are 5 columns that are date relate--"Date As of, Start Date, End Date, Year, Month". Demographic columns are " Sex, Age Group, Race and Hispanic Origin", as country here for all observations are the United States, I don't consider it. There are 19 cancer types.
```
...
```{r}
cancer_tidy<-pivot_longer(cancer_deaths2,cols=c(10:28),names_to = "Cancer Type", values_to = "Count")
cancer_tidy
head(cancer_tidy,5)
...
```{r}
#I have 50504 rows and 11 columns in the table. 53504*11=
count(cancer_tidy,`Count`==0)
cancer_tidy_sparse<-subset(cancer_tidy,(Count!="0"))
cancer_tidy_sparse
...
Group the data by `finalGame_d` and count the number of players with each date as a final game. Sort in descending order. What do you notice about the date 2021-10-03? What is the maximum `finalGame_d` in the data set? How would you explain this result?

```{r}
People
People2<- mutate(People,debut_d=ymd(debut),finalGame_d=ymd(finalGame))
People2
People2 %>%
  group_by(finalGame_d)%>%
  summarize(count=n())%>%
  arrange(desc(count))
  
#There are 488 players joined the final game on 2021-10-03. The maximum 'finalGame_d' is 2021-10-03. It means the last game of 2021 regular season is 2021-10-03.
```
...
```{r}
People3<-mutate(People2,career_length=time_length((difftime(`finalGame_d`,`debut_d`)),"years"))
ggplot(People3,aes(x=career_length))+geom_density(fill="#69b3a2", color="#e9ecef", alpha=0.8)+ ggtitle("Density Plot of Career Length")
#A typical career length is around 1 year.
```
...
```{r}
decade<-function(x) {(year(x)-5)%>%
  round(digits = -1)}
People_Decade<-mutate(People3, `Decade_of_debut`=decade(debut))
ggplot(People_Decade,aes(x=Decade_of_debut,y=time_length(career_length, "years")))+geom_boxplot()+geom_jitter()+
    ggtitle("Boxplot of Career Length by decade of debut")

#Prior to 1950, Players with decade of debut prior to 1950 tend to have longer career lengths, while in the decade of 1950, It appears more players with longer career lengths. While after 1950, the length of career decreases. It might because many of them are new players that just joined in, and the newest game was in 2021, so they couldn't have longer career lengths since it's still ongoing.
```
...
```{r}
People_Decade %>%
filter(!is.na(debut_d))%>%
arrange(desc(career_length))
#Nicolas Altrock-35.21years. From 1898 to 1906 he played regularly, while an arm injury in 1906 ruined his career. He had sporadic appearances after that until his last game in 1953. With Al Schacht, he performed comedy routines.
```
...
```{r}
People_Decade %>%
filter(finalGame_d == "2021-10-03")%>%
  arrange(desc(career_length))
#Jose Alberto Pujols has the longest career of 20.5 years.
```
